...
function channelListener(msg) {
  if (typeof msg === 'string') {
    ...
    console.log('[cordova] "unexpected object type: ' + msg);
    console.log('msg: ', msg);
  }
};
...
function startupCallback(err) {
  if (err) {
    ...
    nodejs.channel.post('objEvent', ['strings', 'from', 'cordova', {nestedObject:[1,2,3]}]);
    nodejs.channel.post('booleanEvent', false);
  }
};
...
function startNodeProject() {
  ...
  nodejs.channel.on('foo', channelListener);
  nodejs.channel.on('func', channelListener);
